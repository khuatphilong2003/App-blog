...
const RegisterAdmin = (props) => {
  let uriUser = API+"tbUsers?username=" // Đia chỉ lấy user 
  ...
  const onGetData = ()=>{
    ...
    fetch(url_checkRegister).then((res)=>{return res.json()}).then((res_json)=>setListUser(res_json));
  }
  useEffect(()=>{
    onGetData();
  },[])
            ...
            <TextInput
              ...
              onChangeText={(Text) => {
                if(checkPass(Text) == false){
                  setErrorPass("Mật khẩu từ 6 đến 20 ký tự chứa ít nhất một chữ số, {\n}một chữ hoa và một chữ thường");
                }
